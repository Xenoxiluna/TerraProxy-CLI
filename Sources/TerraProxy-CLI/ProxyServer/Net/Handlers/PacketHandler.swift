...
func HandlePacket(channel: Channel, bb: InboundIn, connection: PlayerConnection, _ direction: PacketDirection) {
    ...
    do{
        try packet.decode()
        print("Decode Successful on \(packet.getType()) for Player \(connection.playerId)!")
    }catch{
        print("Decode failed on \(packet.getType()) for Player \(connection.playerId)...")
        print("")
        channel.writeAndFlush(NIOAny.init(bb), promise: nil)
        return
    }
...
func debugPacketInfo(_ packet: TerrariaPacket, _ connection: PlayerConnection, _ direction: PacketDirection){
    /**
            TODO
